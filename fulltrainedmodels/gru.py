...
class GRUMulti(nn.Module):
    def __init__(self, n_features, hid, n_layers, dropout):
        super().__init__()
        self.gru = nn.GRU(input_size=n_features, hidden_size=hid, num_layers=n_layers,
                          batch_first=True, dropout=dropout if n_layers > 1 else 0.0)
        self.norm = nn.LayerNorm(hid)
        self.head = nn.Sequential(nn.Linear(hid, hid // 2), nn.GELU(), nn.Dropout(dropout), nn.Linear(hid // 2, 4))
    def forward(self, x):
        _, h = self.gru(x)
...
def objective(trial):
    ...
    m = GRUMulti(N_FEATURES, hid, n_layers, dropout).to(DEVICE)
...
final_model = GRUMulti(N_FEATURES, best['hid'], best['n_layers'], best['dropout']).to(DEVICE)
...
torch.save(final_model.state_dict(), 'gru_model.pt')
np.save('gru_point_preds.npy', y_p_point); np.save('gru_q10_preds.npy', y_p_q10)
np.save('gru_q50_preds.npy', y_p_q50);    np.save('gru_q90_preds.npy', y_p_q90)
np.save('gru_targets.npy', y_te_global);  np.save('gru_test_timestamps.npy', ts_te_global)
np.save('gru_oos_point_preds.npy', y_oos_p_point); np.save('gru_oos_q10_preds.npy', y_oos_p_q10)
np.save('gru_oos_q50_preds.npy', y_oos_p_q50);    np.save('gru_oos_q90_preds.npy', y_oos_p_q90)
np.save('gru_oos_targets.npy', y_oos_t);  np.save('gru_oos_timestamps.npy', ts_oos_final)

with open('gru_results.json', 'w') as f:
    json.dump({
        'model': 'GRU Multi-Output',
...
print("\nFertig! gru_results.json gespeichert.")