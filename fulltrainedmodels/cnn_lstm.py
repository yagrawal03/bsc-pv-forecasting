...
def load_batch(spots, fit_scaler=False):
    ...
    df_tr = df.loc[split == 0].copy()
    df_va = df.loc[split == 1].copy()
    df_te = df.loc[split == 2].copy()
    segs_tr = build_segments(df_tr)
    segs_va = build_segments(df_va)
    segs_te = build_segments(df_te)
...
class CNNLSTMMulti(nn.Module):
    def __init__(self, n_features, cnn_ch, hid, n_layers, dropout):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv1d(n_features, cnn_ch, kernel_size=3, padding=1),
            nn.BatchNorm1d(cnn_ch), nn.ReLU(),
        )
        self.lstm = nn.LSTM(input_size=cnn_ch, hidden_size=hid, num_layers=n_layers,
                            batch_first=True, dropout=dropout if n_layers > 1 else 0.0)
        self.head = nn.Sequential(
            nn.Linear(hid, hid // 2), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hid // 2, 4),
        )
    def forward(self, x):
...
class CNNLSTMSingle(nn.Module):
    def __init__(self, n_features, cnn_ch, hid, n_layers, dropout):
        ...
        self.lstm = nn.LSTM(input_size=cnn_ch, hidden_size=hid, num_layers=n_layers,
                            batch_first=True, dropout=dropout if n_layers > 1 else 0.0)
        self.head = nn.Sequential(
            nn.Linear(hid, hid // 2), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hid // 2, 1),
        )
    def forward(self, x):
        x = self.conv(x.permute(0, 2, 1)).permute(0, 2, 1)
        _, (h, _) = self.lstm(x)
...
def pinball_loss_single(pred, target, alpha):
    e = target - pred
...
def combined_loss(pred, target):
    return (
        ...
        + pinball_loss_single(pred[:, 1], target, 0.1)
        + pinball_loss_single(pred[:, 2], target, 0.5)
        + pinball_loss_single(pred[:, 3], target, 0.9)
    )

def predict_loader_multi(model, loader):
    preds = []
    model.eval()
    with torch.no_grad():
        for xb, _ in loader:
...
def predict_loader_single(model, loader):
    preds = []
    model.eval()
    with torch.no_grad():
        for xb, _ in loader:
            preds.append(model(xb.to(DEVICE, non_blocking=True)).cpu().numpy())
    if not preds: return np.empty((0,), dtype=np.float32)
    return np.maximum(0.0, np.concatenate(preds, axis=0))
...
def objective(trial):
    ...
    m = CNNLSTMMulti(N_FEATURES, cnn_ch, hid, n_layers, dropout).to(DEVICE)
...
print("\n=== Point Model ===")
final_model = CNNLSTMMulti(N_FEATURES, best['cnn_ch'], best['hid'],
...
for epoch in range(8):
    ...
    if val_loss < best_val - 1e-5:
        ...
        no_improve += 1
        if no_improve >= 2: print(f"Early stopping Epoch {epoch+1}"); break

final_model.load_state_dict(best_state)

# Test Predictions Point
...
for i, batch_spots in enumerate(spot_batches):
    ...
    test_dl = DataLoader(SpotSeqDS(X_te, y_te, segs_te, best['slen'], stride=TEST_STRIDE),
                         batch_size=EVAL_BATCH_SIZE, shuffle=False, pin_memory=PIN_MEMORY, num_workers=2)
    all_preds.append(predict_loader_multi(final_model, test_dl))
...
print("\nBerechne Permutation Importance...")
baseline_dl = DataLoader(SpotSeqDS(X_te0, y_te0, segs_te0, best['slen'], stride=1),
                         batch_size=EVAL_BATCH_SIZE, shuffle=False, pin_memory=PIN_MEMORY, num_workers=2)
baseline_preds = predict_loader_multi(final_model, baseline_dl)[:, 0]
...
for feat_idx, feat_name in enumerate(FEATURES):
    ...
    perm_dl = DataLoader(SpotSeqDS(X_perm, y_te0, segs_te0, best['slen'], stride=1),
                         batch_size=EVAL_BATCH_SIZE, shuffle=False, pin_memory=PIN_MEMORY, num_workers=2)
    y_perm = predict_loader_multi(final_model, perm_dl)[:, 0]
...
oos_dl = DataLoader(SpotSeqDS(X_oos, y_oos, segs_oos, best['slen'], stride=OOS_STRIDE),
                    batch_size=EVAL_BATCH_SIZE, shuffle=False, pin_memory=PIN_MEMORY, num_workers=2)
preds_oos_point = predict_loader_multi(final_model, oos_dl)[:, 0]
...
oos_mae  = float(mean_absolute_error(y_oos_t, preds_oos_point))
oos_rmse = float(np.sqrt(mean_squared_error(y_oos_t, preds_oos_point)))
oos_r2   = float(r2_score(y_oos_t, preds_oos_point))
print(f"OOS → MAE={oos_mae:.6f} | RMSE={oos_rmse:.6f} | R²={oos_r2:.6f}")

torch.save(final_model.state_dict(), '/kaggle/working/cnnlstm_model.pt')
np.save('/kaggle/working/cnnlstm_point_preds.npy', y_p_point)
np.save('/kaggle/working/cnnlstm_targets.npy', y_te_global)
np.save('/kaggle/working/cnnlstm_test_timestamps.npy', ts_te_global)
np.save('/kaggle/working/cnnlstm_oos_point_preds.npy', preds_oos_point)
np.save('/kaggle/working/cnnlstm_oos_targets.npy', y_oos_t)
np.save('/kaggle/working/cnnlstm_oos_timestamps.npy', ts_oos_final)
del final_model, best_state; gc.collect()

# ── 3 Separate Quantile Modelle ───────────────────────────────────────
q_preds_te  = {}
q_preds_oos = {}

for q in QUANTILES:
    qname = f'q{int(q*100)}'
    print(f"\n=== CNN-LSTM Quantile {qname} (alpha={q}) ===")

    q_model = CNNLSTMSingle(N_FEATURES, best['cnn_ch'], best['hid'],
                             best['n_layers'], best['dropout']).to(DEVICE)
    q_optimizer = torch.optim.Adam(q_model.parameters(), lr=best['lr'])
    q_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(q_optimizer, patience=2, factor=0.5)
    best_val_q, no_improve_q, best_state_q = float('inf'), 0, None
    ...
    for epoch in range(8):
        ...
        for i, batch_spots in enumerate(spot_batches):
            ...
            for xb, yb in train_dl:
                ...
                loss = pinball_loss_single(q_model(xb), yb, q)
                loss.backward(); q_optimizer.step()
                ep_loss += loss.item() * len(xb); ep_n += len(xb)
            q_model.eval()
            with torch.no_grad():
                for xb, yb in val_dl:
        ...
        print(f"  Epoch {epoch+1:02d} | Train: {train_loss:.6f} | Val: {val_loss:.6f}")
        if val_loss < best_val_q - 1e-5:
            best_val_q = val_loss; no_improve_q = 0
            best_state_q = {k: v.detach().cpu().clone() for k, v in q_model.state_dict().items()}
        else:
            no_improve_q += 1
            if no_improve_q >= 2: print(f"  Early stopping Epoch {epoch+1}"); break

    q_model.load_state_dict(best_state_q)
    torch.save(q_model.state_dict(), f'/kaggle/working/cnnlstm_{qname}_model.pt')
    ...
    for i, batch_spots in enumerate(spot_batches):
        ...
        test_dl = DataLoader(SpotSeqDS(X_te, y_te, segs_te, best['slen'], stride=TEST_STRIDE),
                             batch_size=EVAL_BATCH_SIZE, shuffle=False, pin_memory=PIN_MEMORY, num_workers=2)
        all_preds_q.append(predict_loader_single(q_model, test_dl))
        all_targets_q.append(extract_targets(y_te, segs_te, best['slen'], stride=TEST_STRIDE))
        if i > 0: del X_te, y_te, ts_te; gc.collect()

    q_preds_te[qname] = np.concatenate(all_preds_q)
    np.save(f'/kaggle/working/cnnlstm_{qname}_preds.npy', q_preds_te[qname])

    # OOS Predictions
    oos_preds_q = []
    q_model.eval()
    with torch.no_grad():
        for xb, _ in DataLoader(SpotSeqDS(X_oos, y_oos, segs_oos, best['slen'], stride=OOS_STRIDE),
                                batch_size=EVAL_BATCH_SIZE, pin_memory=PIN_MEMORY, num_workers=2):
            oos_preds_q.append(np.maximum(0.0, q_model(xb.to(DEVICE)).cpu().numpy()))
    q_preds_oos[qname] = np.concatenate(oos_preds_q)
    np.save(f'/kaggle/working/cnnlstm_oos_{qname}_preds.npy', q_preds_oos[qname])

    del q_model, best_state_q; gc.collect()

# ── Finale Metriken ───────────────────────────────────────────────────
y_te_global = np.concatenate(all_targets_q)

pinball_scores = {
    'q10': pinball_np(y_te_global, q_preds_te['q10'], 0.1),
    'q50': pinball_np(y_te_global, q_preds_te['q50'], 0.5),
    'q90': pinball_np(y_te_global, q_preds_te['q90'], 0.9),
}
coverage = float(np.mean((q_preds_te['q10'] <= y_te_global) & (y_te_global <= q_preds_te['q90'])))

oos_coverage = float(np.mean((q_preds_oos['q10'] <= y_oos_t) & (y_oos_t <= q_preds_oos['q90'])))
oos_pinball  = {
    'q10': pinball_np(y_oos_t, q_preds_oos['q10'], 0.1),
    'q50': pinball_np(y_oos_t, q_preds_oos['q50'], 0.5),
    'q90': pinball_np(y_oos_t, q_preds_oos['q90'], 0.9),
}

print(f"\n=== QUANTILE TEST ===")
print(f"Pinball: {pinball_scores} | Coverage: {coverage:.3f}")
print(f"OOS Pinball: {oos_pinball} | OOS Coverage: {oos_coverage:.3f}")

with open('/kaggle/working/cnnlstm_results.json', 'w') as f:
    json.dump({
        'model': 'CNN-LSTM',
        'split': {'train': f'bis {train_end}', 'val': 'Nov–Dez 2025', 'test': 'Jan 2026 – Ende'},
        'n_train_spots': len(all_spots), 'n_oos_spots': len(oos_spots),
        'n_batches': len(spot_batches),
        'mae': mae, 'rmse': rmse, 'r2': r2,
        'best_params': best, 'best_val_loss': float(best_val),
        'n_params': sum(p.numel() for p in CNNLSTMMulti(
            N_FEATURES, best['cnn_ch'], best['hid'], best['n_layers'], best['dropout']).parameters()),
        'train_time': round(time.time() - t0, 2),
        'train_losses': [float(x) for x in train_losses],
        'val_losses':   [float(x) for x in val_losses],
        'permutation_importance': perm_importance,
        'quantiles': {
            'method': 'separate_pinball_models',
            'alphas': QUANTILES,
        ...
        'oos': {
            'mae': oos_mae, 'rmse': oos_rmse, 'r2': oos_r2,
            'coverage_80pct': oos_coverage,
            'pinball_loss': oos_pinball,
            'n_spots': len(oos_spots),
            'n_rows': int(len(y_oos_t)),
            'stride': OOS_STRIDE,
        },
    }, f, indent=2)
print("\nFertig! cnnlstm_results.json gespeichert.")