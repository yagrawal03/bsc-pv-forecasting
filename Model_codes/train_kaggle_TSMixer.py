...
def objective(trial):
    try:
        ...
        dl  = DataLoader(SpotSeqDS(X_tr, y_tr, segs_tr, slen, stride=96), batch_size=256, shuffle=True, pin_memory=True, num_workers=2)
        dva = DataLoader(SpotSeqDS(X_va, y_va, segs_va, slen, stride=slen), batch_size=512, pin_memory=True, num_workers=2)
...
final_model = TSMixer(best['slen'], len(FEATURES), best['d_model'], best['n_blocks'], best['dropout']).to(DEVICE)
optimizer   = torch.optim.Adam(final_model.parameters(), lr=best['lr'])
scheduler   = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=3, factor=0.5)
train_dl = DataLoader(SpotSeqDS(X_tr, y_tr, segs_tr, best['slen'], stride=48), batch_size=256, shuffle=True, pin_memory=True, num_workers=2)
val_dl   = DataLoader(SpotSeqDS(X_va, y_va, segs_va, best['slen'], stride=best['slen']), batch_size=512, pin_memory=True, num_workers=2)
...
final_model.eval()
TEST_STRIDE = 1
test_dl = DataLoader(SpotSeqDS(X_te, y_te, segs_te, best['slen'], stride=TEST_STRIDE), batch_size=512, pin_memory=True, num_workers=2)
preds = []
with torch.no_grad():
...
print(f"len(y_p)={len(y_p)} | len(y_t)={len(y_t)} | Gleich={len(y_p)==len(y_t)}")
...
OOS_STRIDE = best['slen']
oos_dl = DataLoader(SpotSeqDS(X_oos, y_oos, segs_oos, best['slen'], stride=OOS_STRIDE), batch_size=512, pin_memory=True, num_workers=2)
preds_oos = []
with torch.no_grad():
    for xb, _ in oos_dl: preds_oos.append(final_model(xb.to(DEVICE)).cpu().numpy())
y_oos_p = np.maximum(0, np.concatenate(preds_oos))
y_oos_t = extract_targets(y_oos, segs_oos, best['slen'], stride=OOS_STRIDE)
print(f"len(y_oos_p)={len(y_oos_p)} | len(y_oos_t)={len(y_oos_t)} | Gleich={len(y_oos_p)==len(y_oos_t)}")
...
torch.save(final_model.state_dict(), 'tsmixer_model.pt')
np.save('tsmixer_preds.npy', y_p); np.save('tsmixer_targets.npy', y_t)
np.save('tsmixer_oos_preds.npy', y_oos_p); np.save('tsmixer_oos_targets.npy', y_oos_t)

with open('tsmixer_results.json', 'w') as f:
    json.dump({
        'model': 'TSMixer',
        ...
        'oos': {'mae': oos_mae, 'rmse': oos_rmse, 'r2': oos_r2,
                'n_spots': len(oos_spots), 'n_rows': int(len(y_oos_t)), 'stride': OOS_STRIDE}
    }, f, indent=2)
print("Fertig! tsmixer_results.json gespeichert.")