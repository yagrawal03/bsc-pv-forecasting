...
final_model = GRUMod(len(FEATURES), best['hid'], best['n_layers'], best['dropout']).to(DEVICE)
...
final_model.eval()
test_dl = DataLoader(SpotSeqDS(X_te, y_te, segs_te, best['slen'], stride=1),
                     batch_size=512, pin_memory=True, num_workers=2)
preds = []
with torch.no_grad():
    for xb, _ in test_dl:
...
mae, rmse, r2 = float(mean_absolute_error(y_t, y_p)), float(np.sqrt(mean_squared_error(y_t, y_p))), float(r2_score(y_t, y_p))
print(f"\n=== ERGEBNISSE ===\nMAE:  {mae:.6f}\nRMSE: {rmse:.6f}\nR²:   {r2:.6f}")
...
y_oos_all = np.maximum(0, np.concatenate(preds_oos))  # shape (N, 4): point, q10, q50, q90
y_oos_p   = y_oos_all[:, 0]                           # point prediction
...
np.save('gru_oos_point_preds.npy',  y_oos_p)
np.save('gru_oos_targets.npy',      y_oos_t)
np.save('gru_oos_timestamps.npy',   ts_oos_t)

torch.save(final_model.state_dict(), 'gru_model.pt')
with open('gru_results.json', 'w') as f:
    json.dump({
        'model': 'GRU (Multi-Output, alle Spots)',
