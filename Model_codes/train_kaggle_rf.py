...
def objective(trial):
    params = dict(n_estimators=trial.suggest_int('n_estimators', 50, 200),
    ...
    m.fit(X_tr[:2_000_000], y_tr[:2_000_000])
...
feature_importance = dict(sorted(zip(FEATURES, [float(x) for x in final_model.feature_importances_]), key=lambda x: x[1], reverse=True))
...
X_oos = scaler.transform(df_oos[FEATURES].values.astype(np.float32))  # FIXED
...
np.save('rf_preds.npy', y_p); np.save('rf_targets.npy', y_te)
np.save('rf_oos_preds.npy', y_oos_p); np.save('rf_oos_targets.npy', y_oos)

with open('rf_results.json', 'w') as f:
    json.dump({'model': 'RandomForest', 'mae': mae, 'rmse': rmse, 'r2': r2,
               ...
               'train_time': round(time.time() - t0, 2), 'feature_importance': feature_importance,
               'oos': {'mae': oos_mae, 'rmse': oos_rmse, 'r2': oos_r2,
                       'n_spots': len(oos_spots), 'n_rows': int(len(y_oos))}}, f, indent=2)
print("Fertig! rf_results.json gespeichert.")